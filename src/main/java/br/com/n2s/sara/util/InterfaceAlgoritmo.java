...
public class InterfaceAlgoritmo {
	...
	public static List<AvaliaTrabalho> distribuirPorTrilha(Trilha t, int numCorrecoes){
		...
		//preenchendo a linha onde so existe se o tabalho n�o pertencer aquele autor
		for (int i=1; i<=trabalhos.size();i++) {//contador da linha, inicia na linha 1 e vai at� igual aos total de trabalhos
...
//				}
		...
		//Falta terminar de implementar pq at� aqui ele executa, falta remover e pegar o grafo e transformar  novamente em duas listas. 
		//Transformando o grafo em Lista dnv
		for (int i=0; i<=avaliadores.size()-1;i++) {//contador da linha, inicia na linha 1 e vai at� igual aos total de trabalhos
			for(int j=1;j<=trabalhos.size();j++) {//contador da linha, inicia na linha 1 e vai at� igual aos total de trabalhos
...
//				}
	...
	public static List<AvaliaTrabalho> distribuirTrabalhosRecurso(Trilha t, int numCorrecoes){
		ArrayList<AvaliaTrabalho> distribuidos = new ArrayList<AvaliaTrabalho>();
		ArrayList<Usuario> avaliadores = t.getAvaliadores();
		ArrayList<Trabalho> trabalhos = new DAOTrabalho().readTrilha(t.getIdTrilha(), StatusTrabalho.ENVIADO);
		int grafo [][] = new int [trabalhos.size()+avaliadores.size()+2][trabalhos.size()+avaliadores.size()+2];
		float a = (trabalhos.size() * t.getQtdCorrecoes())/avaliadores.size();
		int fatorCorrecao = Math.round(a);
		int tamanho = avaliadores.size()+trabalhos.size()+2;
	 ...
	 * o tabalho n�o pertencer aquele autor for (int i=1; i<=trabalhos.size();i++)
	 * {//contador da linha, inicia na linha 1 e vai at� igual aos total de
	 ...
	 * at� aqui ele executa, falta remover e pegar o grafo e transformar novamente
...
//				}
	...
	public static List<AvaliaTrabalho> distribuPorEventoRecurso(Evento e, int numCorrecoes){
		ArrayList<AvaliaTrabalho> distribuidos = new ArrayList<AvaliaTrabalho>();
		ArrayList<Usuario> avaliadores = (ArrayList<Usuario>) new DAOAvaliaEvento().buscarPorEvento(e.getIdEvento());
		ArrayList<Trabalho> trabalhos = new ArrayList<Trabalho>();
		e.setTrilhas(new DAOTrilha().readById(e.getIdEvento()));
		...
		for(Trilha t : e.getTrilhas()) {
			t = new DAOTrilha().getTrilha(t.getIdTrilha());
			ArrayList<Trabalho> trabAdd = new DAOTrabalho().readTrilha(t.getIdTrilha(), StatusTrabalho.ENVIADO);
			if(trabAdd!=null)
				trabalhos.addAll(trabAdd);
		}
		int tamanho = avaliadores.size()+trabalhos.size()+2;
		...
		for (int i=1; i<=trabalhos.size();i++) {//contador da linha, inicia na linha 1 e vai at� igual aos total de trabalhos
			for(int j = trabalhos.size()+1;j<=tamanho-2;j++) {
				...
				if(verficaAutores(avaliadores.get(posA).getCpf(),trabalhos.get(posT))) {
					...
					if(Facade.isAvaliador(trabalhos.get(posT), avaliadores.get(posA).getCpf())) {
						grafo[i][j]=1;
					}else {
						grafo[i][j]=0;
					}
